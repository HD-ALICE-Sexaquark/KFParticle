...
class KFParticleBase {
 ...
 public:
  ...
  int GetNDF  () const { return fNDF;  } ///< Returns number of decrease of freedom.
  ...
  void         SetIdTruth(int idtru,int qatru=0) {fIdTruth = (unsigned short) idtru; fQuality = (unsigned short) qatru;}
...
void PrintNonSymMatrix(std::string_view name, D arr[N]) const {
  ...
  for (int i{0};i<N;++i){
    std::cout << arr[i] ;
    counter++;
    if (counter < std::sqrt(N))std::cout << "    ";
    else {std::cout << '\n';
    counter=0;}
  }
}
