...
void KFParticle::Create(const float Param[], const float Cov[], int Charge, float mass) {
    float C[21];
...
// \param[in] Charge - charge of the particle in elementary charge units
// \param[in] mass - the mass hypothesis
void KFParticle::Create(const double Param[], const double Cov[], int Charge, float mass) {
    float P[6];
...
KFParticle::KFParticle(const KFPTrack &track, const float mass) : KFParticleBase() {
    ...
    for (int iF = 0; iF < 10; iF++) SetFieldCoeff(track.GetFieldCoeff()[iF], iF);
#endif
}
...
// Calculate the DCA distance from a vertex together with the error in the XY plane.
// Return "true" if calculation is failed, "false" if both value and the error are well defined.
...
bool KFParticle::GetDistanceFromVertexXY(const float vtx[], const float Cv[], float &val, float &err) const {
    ...
    err = h0 * (h0 * GetCovariance(0, 0) + h1 * GetCovariance(0, 1) + h3 * GetCovariance(0, 3) + h4 * GetCovariance(0, 4)) +
...
bool KFParticle::GetDistanceFromVertexXY(const float vtx[], float &val, float &err) const { return GetDistanceFromVertexXY(vtx, 0, val, err); }

// Calculate the DCA distance from a vertex in the KFParticle format together with the error in the XY plane.
// Return "true" if calculation is failed, "false" if both value and the error are well defined.
// \param[in] Vtx - the vertex in the KFParticle format
// \param[out] val - the distance in the XY plane to the vertex
// \param[out] err - the error of the calculated distance, takes into account errors of the particle and vertex
bool KFParticle::GetDistanceFromVertexXY(const KFParticle &Vtx, float &val, float &err) const {
    return GetDistanceFromVertexXY(Vtx.fP, Vtx.fC, val, err);
}

#ifdef HomogeneousField
// Calculate the DCA distance from a vertex in the KFPVertex format together with the error in the XY plane.
// Return "true" if calculation is failed, "false" if both value and the error are well defined.
// \param[in] Vtx - the vertex in the KFPVertex format
// \param[out] val - the distance in the XY plane to the vertex
// \param[out] err - the error of the calculated distance, takes into account errors of the particle and vertex
bool KFParticle::GetDistanceFromVertexXY(const KFPVertex &Vtx, float &val, float &err) const {
    return GetDistanceFromVertexXY(KFParticle(Vtx), val, err);
}
#endif

// Return the DCA distance from a vertex in the XY plane.
// \param[in] vtx[2] - { X, Y } coordinates of the vertex
...
// Return the DCA distance from a vertex in the KFParticle format in the XY plane.
//  \param[in] Vtx - the vertex in the KFParticle format
float KFParticle::GetDistanceFromVertexXY(const KFParticle &Vtx) const { return GetDistanceFromVertexXY(Vtx.fP); }

#ifdef HomogeneousField
// Return the DCA distance from a vertex in the KFParticle format in the XY plane.
// \param[in] Vtx - the vertex in the KFPVertex format
float KFParticle::GetDistanceFromVertexXY(const KFPVertex &Vtx) const { return GetDistanceFromVertexXY(KFParticle(Vtx).fP); }
#endif

// Return the DCA distance between the current and the second particles in the XY plane.
...
// Return sqrt(Chi2/ndf) deviation from other particle in the XY plane.
// \param[in] p - the second particle
float KFParticle::GetDeviationFromParticleXY(const KFParticle &p) const {
    ...
    float sigmaS = .1 + 10. * std::sqrt((d[0] * d[0] + d[1] * d[1]) / (mP1[3] * mP1[3] + mP1[4] * mP1[4]));
    ...
    return GetDeviationFromVertexXY(mP1, mC1) * std::sqrt(2. / 1.);
}

// Return sqrt(Chi2/ndf) deviation from the vertex in the XY plane.
// \param[in] vtx[2] - { X, Y } coordinates of the vertex
// \param[in] Cv[3] - lower-triangular part of the covariance matrix of the vertex
...
// Return sqrt(Chi2/ndf) deviation from the vertex in the KFParticle format in the XY plane.
// \param[in] Vtx - the vertex in the KFParticle format
float KFParticle::GetDeviationFromVertexXY(const KFParticle &Vtx) const { return GetDeviationFromVertexXY(Vtx.fP, Vtx.fC); }

#ifdef HomogeneousField
// Return sqrt(Chi2/ndf) deviation from the vertex in the KFPVertex format in the XY plane.
// \param[in] Vtx - the vertex in the KFPVertex format
...
void KFParticle::GetParametersAtPoint(const float *point, const float *pointCov, float *m, float *mV) {
    ...
    float V1Tmp[36];
    for (int i = 0; i < 36; i++) V1Tmp[i] = 0.f;
    KFParticle::MultQSQt(F1, pointCov, V1Tmp, 6);

    for (int iC = 0; iC < 21; iC++) mV[iC] += V1Tmp[iC];
}

// Return the opening angle between the current and the second particle in 3D.
// \param[in] p - the second particle
...
// Return the opening angle between the current and the second particle in the XY plane.
// \param[in] p - the second particle
...
// Return the opening angle between the current and the second particle in the RZ plane, R = sqrt(X*X+Y*Y).
// \param[in] p - the second particle
...
// Return the Pseudo Proper Time of the decay = (r*pt) / |pt| * M/|pt|
...
float KFParticle::GetPseudoProperDecayTime(const KFParticle &pV, const float &mass, float *timeErr2) const {
    ...
    if (timeErr2) {
        ...
        *timeErr2 = f5 * mC55 * f5 + f5 * mC54 * f4 + f4 * mC44 * f4 + f3 * mC33 * f3 + f3 * mC32 * f2 + f3 * mC31 * f1 + f3 * mC30 * f0 +
                    f2 * mC22 * f2 + f2 * mC21 * f1 + f2 * mC20 * f0 + f1 * mC11 * f1 + f1 * mC10 * f0 + f0 * mC00 * f0;
    }
